#!/usr/bin/env bash
set -euo pipefail
talosctl gen config sakura https://k8s-controlplane.strypsteen.com:6443 --config-patch @"$(dirname "$0")"/k8s-infra/talos.yaml --with-secrets secrets.yaml --kubernetes-version 1.34.1 --force
